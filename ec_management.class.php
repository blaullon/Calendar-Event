	...
	*/
	...
	function display() {
		...
		if(isset($_POST['EC_editEventFormSubmitted'])) {
			...
			$this->editEvent($id, stripslashes($title), stripslashes($location), $linkout, stripslashes($description), stripslashes($eventType), $startDate, $startTime, $endDate, $endTime, $accessLevel, $postID);
			$splitDate = split("-", $startDate);
			$this->month = $splitDate[1];
			$this->year = $splitDate[0];
		}
